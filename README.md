...
При старте контейнера `backend` автоматически выполняется `alembic upgrade head`, затем поднимается API. Сервис `telegram-ingestion` ждёт готовности `backend`, чтобы в БД уже были таблицы.
